...
import { TOLERANCE_MS } from "../../constants";
...
export function TooltipPanel({ activeX, startTime, currentSpan, eventsNear, isFrozen }: TooltipPanelProps) {
  return (
    <div className="dbg2-tooltip-panel">
      {activeX == null ? (
        ...
        <div className="dbg2-tooltip">
          <div className="dbg2-tooltip-time">{activeX != null ? formatTimeIso(new Date(startTime + Math.round(activeX / 40) * 1000).toISOString()) : "—"}</div>
          {currentSpan && (
          ...
          <div className="dbg2-tooltip-list">
            {eventsNear.length === 0 ? (
              <div className="dbg2-tooltip-none">No events within ±{TOLERANCE_MS}ms</div>
            ) : (
              eventsNear.map((ev) => {
